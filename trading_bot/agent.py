...
class State:
    def __init__(self, balance, invested, candle, lstm_out):
        self.balance = balance
        self.invested = invested
        self.candle = candle
        self.lstm_out = lstm_out

    def get_net_worth(self, btc_price):
        return self.balance + btc_price * self.invested

    def to_array(self):
        return np.array([[self.balance, self.invested, *self.candle, self.lstm_out]])

    def size(self):
        return 9

    def __str__ (self):
        return f' Balance: {self.balance}\n Invested: {self.invested}\n Candle: {self.candle}\n LstmPrediction: {self.lstm_out}\n'
...
class Agent:
    ...
    def __init__(self, state_size, strategy="t-dqn", reset_every=1000, pretrained=False, model_name=None):
        ...
        self.action_size = 4           		# [sit, buy, sell, amount]
    ...
    def choose_action(self, state, is_eval=False):
        ...
        if not is_eval and random.random() <= self.epsilon:
            ...
            amount = np.random.random(1)[0]
        else:
            model_output = self.model.predict(state.to_array())[0]
            action_probs = model_output[0:2]
            action = np.argmax(action_probs)
            amount = model_output[3]
